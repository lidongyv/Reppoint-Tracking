...
# ['calib', 'disparity',  'label_02', 'splits', 'velodyne', 'velodyne_img', 
...
for i in range(len(data)):
	print(i,'in',len(data))
	info=data[i]
	if info['filename'].split('/')[0]!=video_name:
		video_length=1
		video_name=info['filename'].split('/')[0]
		max_id=-1
		max_id_iou=-1
		max_id_flow=-1
	else:
...
with open(os.path.join('./result/','iou_'+jsonfile_name),'w+',encoding='utf-8') as f:
	data=json.dump(json_data_iou,f)
with open(os.path.join('./result/','flow_'+jsonfile_name),'w+',encoding='utf-8') as f:
