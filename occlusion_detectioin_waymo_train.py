...
for i in range(len(v_video)):
	...
	if len(v_video[i]['occluded'])/v_video[i]['length']>0.3 and vvcase_count[0]*8<np.sum(vvcase_count[1:]):
...
for i in range(len(p_video)):
	...
	if len(p_video[i]['occluded'])/p_video[i]['length']>0.25 and ppcase_count[0]*5<np.sum(ppcase_count[1:]):
...
for i in range(len(vp_video)):
	...
	if len(vp_video[i]['occluded'])/vp_video[i]['length']>0.25 and vpcase_count[0]*2<np.sum(vpcase_count[1:]):
...
mmcv.dump(test_data_all,os.path.join(data_path,'occlusion_train_waymo_zxn_02111423.json'))
# mmcv.dump(train_data_all,os.path.join(data_path,'occlusion_train.json'))
mmcv.dump(test_data_v,os.path.join(data_path,'occlusion_train_waymo_zxn_02111423_v.json'))
mmcv.dump(test_data_p,os.path.join(data_path,'occlusion_train_waymo_zxn_02111423_p.json'))
mmcv.dump(test_data_vp,os.path.join(data_path,'occlusion_train_waymo_zxn_02111423_vp.json'))
