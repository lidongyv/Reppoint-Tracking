...
def trackcd(cd,max_id,dis_thres):
	...
	if (id_check[id]==idx).all() and (id[id_check]==idc).all():
		id=np.where(value>dis_thres*0.8,-1,id)
		return id
	else:
		count=np.bincount(id)
		while(np.max(count)>1):
...
#  'kitti_val.json']
jsonfile_name='reppoint_kitti_val.json'
...
for i in range(len(data)):
	...
	if np.array(info['ann']['bboxes']).shape[0]==0:
		...
		result={"video_id":info['video_id'],"filename":os.path.join(info['filename']), \
			"ann":{"bboxes":info['ann']['bboxes'],"labels":[], \
				"track_id":(boxid+max_id_flow+1).tolist()}}
		json_data_flow.append(result)
	...
	if video_length==1 or max_id==-1:
		...
		distance_thres=np.max(np.abs(boxt1[:,:2]-boxt1[:,-2:]),axis=1)
		...
		cd_id=trackcd(cd,max_id_iou,distance_thres)
		...
		json_data_iou.append(result)
		#check the flow_result
		refer_id=json_data_flow[-1]['ann']['track_id']
		transform=get_transform(flow,c0)
		c1t=c0+transform
		boxt1t=boxt0+0
		boxt1t[:,::2]+=transform[:,0][:,None]
		boxt1t[:,1::2]+=transform[:,1][:,None]
		iou=bbox_overlaps(boxt1,boxt1t)
		cd=center_dist(c1,c1t)
		iou_id=trackiou(iou,max_id_flow)
		cd_id=trackcd(cd,max_id_flow,distance_thres)
		boxid=iou_cd(iou_id,cd_id,refer_id,max_id_flow)
		max_id_flow=np.max(np.maximum(boxid,max_id_flow))
		print('flow id:',boxid,boxid.shape)
		result={"video_id":info['video_id'],"filename":os.path.join(info['filename']), \
			"ann":{"bboxes":boxt1.tolist(),"labels":info['ann']['labels'], \
				"track_id":boxid.tolist()}}
		json_data_flow.append(result)
...
with open(os.path.join('./result/','reppoint_iou_'+jsonfile_name),'w+',encoding='utf-8') as f:
	data=json.dump(json_data_iou,f)
with open(os.path.join('./result/','reppoint_flow_'+jsonfile_name),'w+',encoding='utf-8') as f:
	data=json.dump(json_data_flow,f)
