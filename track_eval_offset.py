	...
	for i in range(-1,-11,-1):
		...
		if i==-1 or max_id==-1:
			...
			for m in range(box_r.shape[0]):
				...
				for n in range(len(box_iou)):
					...
					if np.max(box_iou[n][m])>=0.5:
						...
						continue
					iou_weight=bbox_overlaps(box_r[m].reshape(1,5),record_s[n]['ann']['bboxes']).reshape(-1)
					iou_weight=np.where(iou_weight<0.5,0,1)
					#yx
					loc=loc_s[n][select]
					#xy
					...
					if np.max(mask)>=0.2:
						...
						if np.max(support_select)>=0.2:
							#all related suport box
							support_index=np.where(support_select>=0.2)[0]
							...
							for o in range(len(support_index)):
								...
								if np.max(iou_support)>0.5:
									max_support_index=np.argmax(iou_support)
								else:
									support_labels.append(-1)
									support_score.append(-1)
									continue
								if iou_weight[max_support_index]==1:
									support_labels.append(record_s[n]['ann']['track_id'][max_support_index])
									support_score.append(support_select[support_index[o]])
								else:
									support_labels.append(-1)
									support_score.append(-1)
						else:
						...
						if np.max(support_select)==1:
							support_index=np.where(support_select==1)[0]
							# support_index=np.argmax(support_select)
							for o in range(len(support_index)):
			...
			for m in range(len(new_object)):
				iou_weight=bbox_overlaps(box_r[new_object[m]].reshape(1,5),record_s[0]['ann']['bboxes']).reshape(-1)
				if np.max(iou_weight)>0.5:
					labels[new_object[m]]=record_s[0]['ann']['track_id'][np.argmax(iou_weight)]
				else:
					max_id+=1
					labels[new_object[m]]=max_id
			label_free=[]
			box_free=[]
			for n in range(len(record_s[0]['ann']['track_id'])):
				if record_s[0]['ann']['track_id'][n] in labels.tolist():
					continue
				else:
					label_free.append(record_s[0]['ann']['track_id'][n])
					box_free.append(record_s[0]['ann']['bboxes'][n])
			for m in range(labels.shape[0]):
				id_check=np.where(labels==labels[m])[0]
				# print(id_check)
				# print(box_r[id_check])
				if len(id_check)<=1:
					continue
				else:
					check_label=np.where(record_s[0]['ann']['track_id']==labels[m])[0]
					# print(check_label[0])
					# print(check_label[0][0])
					if len(check_label)>0:
						check_label=check_label[0]
						# print(labels[m])
						# print(record_s[0]['ann']['track_id'])
						# print(box_r[id_check])
						# print(record_s[0]['ann']['bboxes'][check_label])
						iou=bbox_overlaps(box_r[id_check],[record_s[0]['ann']['bboxes'][check_label]]).reshape(-1)
						keep=np.argmax(iou)
						for n in range(len(id_check)):
							if n==keep:
								continue
							iou=bbox_overlaps(box_r[id_check[n]].reshape(1,5),box_free).reshape(-1)
							if np.max(iou)>0:
								labels[id_check[n]]=label_free[np.argmax(iou)]
							else:
								max_id+=1
								labels[id_check[n]]=max_id
					else:
						check_label=np.array(np.arange(len(record_s[0]['ann']['track_id'])))
						if check_label.shape[0]==0:
							continue
						iou=bbox_overlaps(box_r[id_check],p.array(record_s[0]['ann']['bboxes'])[check_label]).reshape(-1)
						for n in range(len(id_check)):
							iou=bbox_overlaps(box_r[id_check[n]].reshape(1,5),box_free).reshape(-1)
							if np.max(iou)>0:
								labels[id_check[n]]=label_free[np.argmax(iou)]
...
def track_online(refer,agg,support,record,info,max_id):
	...
	if refer.shape[0]==0:
		...
		for m in range(labels.shape[0]):
			...
			if len(id_check)<=1:
				...
				if len(check_label)>0:
					...
					if check_label.shape[0]==0:
						continue
					iou=bbox_overlaps(box_r[id_check,:],np.array(record_s[0]['ann']['bboxes'])[check_label]).reshape(-1)
					for n in range(len(id_check)):
						iou=bbox_overlaps(box_r[id_check[n]].reshape(1,5),box_free).reshape(-1)
						if np.max(iou)>0:
							labels[id_check[n]]=label_free[np.argmax(iou)]
		...
		return info,max_id
out_path='/home/ld/RepPoints/final/epoch13 thres0.3'
out_name=['refer','agg']
for i in range(10):
