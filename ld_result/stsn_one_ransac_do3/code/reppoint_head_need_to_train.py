...
class RepPointsHead(nn.Module):
    ...
    def forward_single(self, x,test=False):
        ...
        #     warp_results.append(self.reppoints_cls_out(warp_feature))
        ...
        # classes=[torch.sum((warp_results[:,0,:,:,:]<0.3).float()*(warp_results[:,1,:,:,:]<0.3).float(),dim=-1)]
        # print(torch.sum(classes[0]>3))
        # for m in range(warp_results.shape[1]):
        #     classes.append(torch.sum((warp_results[:,m,:,:,:]>0.3).float(),dim=-1))
        ...
        print('dcn',torch.sum(cls_out.sigmoid()>0.1))
        out=self.reppoints_cls_out(self.relu(cls_feat)).sigmoid()
        print('direct',torch.sum(out>0.1))
