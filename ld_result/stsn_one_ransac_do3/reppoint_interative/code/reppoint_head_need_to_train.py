...
class RepPointsHead(nn.Module):
    ...
    def forward_single(self, x,test=False):
        ...
        pts_feat = x
        with torch.no_grad():
            x_linear = torch.linspace(-1, 1, x.shape[-2])
            y_linear = torch.linspace(-1, 1, x.shape[-1])
            meshx, meshy = torch.meshgrid((x_linear, y_linear))
            grid_init = torch.stack((meshy, meshx), 2)
            grid_init = grid_init.unsqueeze(0).repeat(x.shape[0],1,1,1).cuda(x.device)
        ...
        check_offset=torch.zeros_like(dcn_offset)-dcn_offset
        
        # out=self.reppoints_cls_out(
        #     self.relu(self.reppoints_cls_conv(cls_feat, check_offset)))
        
        for m in range(9):
            grid_cls_init=grid_init+0
            grid_cls_init[:,:,:,0]=grid_cls_init[:,:,:,0]+offset[:,1+2*m,:,:]/x.shape[-1]
            grid_cls_init[:,:,:,1]=grid_cls_init[:,:,:,1]+offset[:,0+2*m,:,:]/x.shape[-2]
            warp_feature=torch.nn.functional.grid_sample(cls_feat,grid_cls_init)
            warp_results.append(self.reppoints_cls_out(
            self.relu(self.reppoints_cls_conv(warp_feature, check_offset))).sigmoid())
            warp_features.append(warp_feature)
        warp_results=torch.stack(warp_results,dim=-1)
        warp_features=torch.stack(warp_features,dim=-1)
        classes=[torch.sum((warp_results[:,0,:,:,:]<0.1).float()*(warp_results[:,1,:,:,:]<0.1).float(),dim=-1)]

        for m in range(warp_results.shape[1]):
            classes.append(torch.sum((warp_results[:,m,:,:,:]>0.1).float(),dim=-1))
        classes=torch.stack(classes,dim=-1)
        init_class=torch.argmax(classes,dim=-1)
        mask=(init_class>0).float()
        # print(warp_results.shape,init_class.shape)
        print(torch.sum(init_class==0),torch.sum(init_class==1),torch.sum(init_class==2))
        init_class_index1=init_class.view(x.shape[0],1,x.shape[-2],x.shape[-1],1).expand(x.shape[0],1,x.shape[2],x.shape[3],warp_results.shape[4])
        init_class_index0=torch.arange(x.shape[0]).view(x.shape[0],1,1,1,1).expand_as(init_class_index1).to(x.device)
        init_class_index2=torch.arange(x.shape[2]).view(1,1,x.shape[2],1,1).expand_as(init_class_index1).to(x.device)
        init_class_index3=torch.arange(x.shape[3]).view(1,1,1,x.shape[3],1).expand_as(init_class_index1).to(x.device)
        init_class_index4=torch.arange(warp_results.shape[4]).view(1,1,1,1,warp_results.shape[4]).expand_as(init_class_index1).to(x.device)
        # print(warp_results[init_class_index0,init_class_index1,init_class_index2,init_class_index3,init_class_index4].shape)
        init_sample_index=torch.argmax(warp_results[init_class_index0,init_class_index1-1,init_class_index2,init_class_index3,init_class_index4],dim=-1)
        # print(mask.shape,init_sample_index.shape)
        init_sample_index4=init_sample_index.view(warp_feature.shape[0],1,warp_feature.shape[2],warp_feature.shape[3],1). \
            expand(warp_feature.shape[0],warp_feature.shape[1],warp_feature.shape[2],warp_feature.shape[3],1)
        init_sample_index0=torch.arange(warp_feature.shape[0]).view(warp_feature.shape[0],1,1,1,1).expand_as(init_sample_index4).to(x.device)
        init_sample_index1=torch.arange(warp_feature.shape[1]).view(1,warp_feature.shape[1],1,1,1).expand_as(init_sample_index4).to(x.device)
        init_sample_index2=torch.arange(warp_feature.shape[2]).view(1,1,warp_feature.shape[2],1,1).expand_as(init_sample_index4).to(x.device)
        init_sample_index3=torch.arange(warp_feature.shape[3]).view(1,1,1,warp_feature.shape[3],1).expand_as(init_sample_index4).to(x.device)
        init_feature=warp_features[init_sample_index0,init_sample_index1,init_sample_index2,init_sample_index3,init_sample_index4] 
        mask=mask.view(warp_features.shape[0],1,warp_features.shape[2],warp_features.shape[3],1)
        print(init_feature.shape,mask.shape)
        # init_score=warp_results[:,init_class,:,:,init_sample_index]*mask
        # inner_index=((warp_results[:,init_class,:,:,:]-init_score)>-0.1).float()
