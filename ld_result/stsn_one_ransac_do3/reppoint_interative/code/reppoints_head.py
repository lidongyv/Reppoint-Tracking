...
class RepPointsHead(nn.Module):
    ...
    def forward_single(self, x,test=False):
        ...
        cls_out_ori = self.reppoints_cls_out(
            self.relu(self.reppoints_cls_conv(cls_feat, dcn_offset)))
        check_index=torch.ones_like(pts_out_init_grad_mul)
        
        for p in range(10):
            # deactivate
            #check deactivate

            cls_out=cls_out_ori.sigmoid()
            for k in range(9):
                check_offset=pts_out_init_grad_mul*check_index+0
                check_offset[:,2*k:2*k+2,...]=check_offset[:,2*k:2*k+2,...]*0
                check_offset=check_offset-dcn_base_offset
                cls_out_check = self.reppoints_cls_out(
                    self.relu(self.reppoints_cls_conv(cls_feat, check_offset))).sigmoid()

                better_check1=(((cls_out_check>cls_out).float()*(cls_out_check>0.1).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
                better_check2=(((cls_out_check<cls_out).float()*(cls_out_check<0.1).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
                better_check=better_check1+better_check2
                check_index[:,2*k,:,:]=torch.where(better_check>0,check_index[:,2*k,:,:]*0,check_index[:,2*k,:,:])
                check_index[:,2*k+1,:,:]=torch.where(better_check>0,check_index[:,2*k+1,:,:]*0,check_index[:,2*k+1,:,:])
                # print(torch.sum(better_check1),torch.sum(better_check2))

            #deactivate most
            offset_de=pts_out_init_grad_mul*check_index
            dcn_offset = offset_de - dcn_base_offset
            cls_out_de = self.reppoints_cls_out(
                self.relu(self.reppoints_cls_conv(cls_feat, dcn_offset)))
            cls_out_new=cls_out_de.sigmoid()
            better_check1=(((cls_out_new>cls_out).float()*(cls_out_new>0.1).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
            better_check2=(((cls_out_new<cls_out).float()*(cls_out_new<0.1).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
            # print('deactivate',torch.sum(better_check1),torch.sum(better_check2),check_index.sum(1).mean())

            #reactivate
            new_check_index=check_index+0
            for k in range(9):
                check_offset=pts_out_init_grad_mul*new_check_index+0
                check_offset[:,2*k:2*k+2,...]=pts_out_init_grad_mul[:,2*k:2*k+2,...]+0
                check_offset=check_offset-dcn_base_offset
                cls_out_check = self.reppoints_cls_out(
                    self.relu(self.reppoints_cls_conv(cls_feat, check_offset))).sigmoid()
                better_check1=(((cls_out_check>cls_out_new).float()*(cls_out_check>0.1).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
                better_check2=(((cls_out_check<cls_out_new).float()*(cls_out_check<0.1).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
                better_check=better_check1+better_check2
                # print('activate',torch.sum((better_check>0).float()-new_check_index[:,2*k,:,:]))
                # print('activate',torch.sum(torch.where(better_check>0,torch.ones_like(new_check_index[:,2*k,:,:]),new_check_index[:,2*k,:,:])-new_check_index[:,2*k,:,:]))
                new_check_index[:,2*k,:,:]=torch.where(better_check>0,torch.ones_like(new_check_index[:,2*k,:,:]),new_check_index[:,2*k,:,:])
                new_check_index[:,2*k+1,:,:]=torch.where(better_check>0,torch.ones_like(new_check_index[:,2*k+1,:,:]),new_check_index[:,2*k+1,:,:])
                # print(torch.sum(better_check1),torch.sum(better_check2))
            # print(torch.sum(new_check_index!=check_index))
            
            offset_re=pts_out_init_grad_mul*new_check_index
            dcn_offset = offset_re - dcn_base_offset
            cls_out_re = self.reppoints_cls_out(
                self.relu(self.reppoints_cls_conv(cls_feat, dcn_offset)))
            
            cls_out_new=cls_out_re.sigmoid()
            better_check1=(((cls_out_new>cls_out).float()*(cls_out>0.1).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
            better_check2=(((cls_out_new<cls_out).float()*(cls_out<0.1).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
            # print('reactivate',torch.sum(better_check1),torch.sum(better_check2),new_check_index.sum(1).mean())
            cls_out_ori = cls_out_re+0
            check_index=new_check_index+0
            
            
        self.mask.append(new_check_index.data.cpu().numpy())
        ...
        return cls_out_re, pts_out_init, pts_out_refine
