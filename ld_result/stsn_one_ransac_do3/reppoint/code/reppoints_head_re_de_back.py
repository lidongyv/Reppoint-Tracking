...
class RepPointsHead(nn.Module):
    ...
    def forward_single(self, x,test=False):
        ...
        for k in range(9):
            check_offset=pts_out_init_grad_mul+0
            ...
            better_check1=(((cls_out_check>cls_out).float()*(cls_out_check>0.05).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
            better_check2=(((cls_out_check<cls_out).float()*(cls_out_check<0.05).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
        ...
        better_check1=(((cls_out_new>cls_out).float()*(cls_out_new>0.05).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
        better_check2=(((cls_out_new<cls_out).float()*(cls_out_new<0.05).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
        ...
        for k in range(9):
            ...
            cls_out_check = self.reppoints_cls_out(
                self.relu(self.reppoints_cls_conv(cls_feat, check_offset))).sigmoid()
            better_check1=(((cls_out_check>cls_out_new).float()*(cls_out_check>0.05).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
            better_check2=(((cls_out_check<cls_out_new).float()*(cls_out_check<0.05).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
            better_check=better_check1+better_check2
        ...
        better_check1=(((cls_out_new>cls_out).float()*(cls_out>0.05).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
        better_check2=(((cls_out_new<cls_out).float()*(cls_out<0.05).float()).sum(1)>0).float().view(x.shape[0],1,x.shape[2],x.shape[3])
