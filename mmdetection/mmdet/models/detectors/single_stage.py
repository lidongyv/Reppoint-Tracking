...
class SingleStageDetector(BaseDetector):
    ...
    def forward_train(self,
        ...
        embed()
        if img_metas[1]['filename'].split('/')[-2] == 'segment-1943605865180232897_680_000_700_000' and img_metas[1]['filename'].split('/')[-1] == '000082.jpg':
            ...
            former_flow = self.flownet_model(former_sample_pair)
            later_flow = self.flownet_model(later_sample_pair)
            ...
            cv2.imwrite('./temp/1233_img_now_%d.png' % 0, now_b)
            cv2.imwrite('./temp/1233_img_former_%d.png' % 0, former_b)
            cv2.imwrite('./temp/1233_img_later_%d.png' % 0, later_b)

            np.save('./temp/1233_former_pair_%d.npy' % 0, former_flow.detach().cpu().numpy())
            np.save('./temp/1233_later_pair_%d.npy' % 0, later_flow.detach().cpu().numpy())

            #
            for each_index, each_flow in enumerate(former_flow):
                a = each_flow.permute(1,2,0)
                flow_image_former_np = flow_to_image(a)
                cv2.imwrite('./temp/1233_flow_init_former_%d.png'%each_index, flow_image_former_np)
            # 保存原始大小的光流  --- later

            for each_index, each_flow in enumerate(later_flow):
                b = each_flow.permute(1,2,0)
                flow_image_later_np = flow_to_image(b)
                cv2.imwrite('./temp/1233_flow_init_later_%d.png'%each_index, flow_image_later_np)
