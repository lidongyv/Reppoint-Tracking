# -*- coding: utf-8 -*- 
#@Author: Lidong Yu   
#@Date: 2020-01-16 14:34:26  
#@Last Modified by: Lidong Yu  
#@Last Modified time: 2020-01-16 14:34:26
...
def mean_shift(feature):
    refer=feature[:,:1,...]
    k=3
    center=feature[:,:1,...]
    with torch.no_grad():
        index=torch.ones_like(feature[:,:3,...]).long()
        ...
        for i in range(3):
            dist=torch.norm(feature-center,p=2,dim=2)
            nearest=torch.topk(dist,k=k,dim=1,largest=False)[1].unsqueeze(2).expand_as(feature[:,:3,...])
            # print(feature.shape,nearest.shape,center.shape)
            # print(feature[index_b,nearest,index_c,index_h,index_w].shape)
            center=torch.mean(torch.cat([feature[index_b,nearest,index_c,index_h,index_w],refer],dim=1),dim=1).unsqueeze(1)
            # print(feature.shape,nearest.shape,center.shape)
    dist=torch.abs(torch.cat([feature[index_b,nearest,index_c,index_h,index_w],refer],dim=1)-center)
    nearest=torch.topk(dist,k=1,dim=1,largest=False)[1]
    center=torch.cat([feature[index_b,nearest,index_c,index_h,index_w],refer],dim=1) \
        [index_b[:,:1,...],nearest,index_c[:,:1,...],index_h[:,:1,...],index_w[:,:1,...]]
    return center.squeeze(1),nearest
@AGG.register_module
...
@AGG.register_module
class STSN_fuse_cluster(nn.Module):
    def __init__(self,in_channels,out_channels,dcn):
        super(STSN_fuse_cluster,self).__init__()
        self.deformable_groups = dcn.get('deformable_groups', 1)
        self.with_modulated_dcn = dcn.get('modulated', False)
        if not self.with_modulated_dcn:
            conv_op = DeformConv
    ...
    def forward(self,datas,test=False):
        ...
        for i in range(len(datas)):
            ...
            # weight=torch.nn.functional.softmax(weight0,dim=1)
            # agg_feature=torch.sum(feature*weight,dim=1)
            agg_feature,weight=mean_shift(feature)
            agg_out.append(agg_feature)
            if i==0 and weight.device==torch.ones(1).cuda(0).device:
                print('mean_weight:',torch.mean(weight.float()).item())
