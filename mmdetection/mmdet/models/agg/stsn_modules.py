    ...
    def forward_eval(self,datas,test=True):
        ...
        for i in range(len(datas)):
            ...
            for j in range(support.shape[0]):
                ...
                support_out[j].append(tk_feature)
                self.offset[j].append(offset)
            weight=torch.nn.functional.softmax(weight0[:,:support_count,...],dim=1)
            agg_feature=torch.sum(feature[:,:support_count,...]*weight,dim=1)
            agg_out.append(agg_feature)
        for i in range(datas[0].shape[0]-1):
            support_out[i]=tuple(support_out[i])
        out=[tuple(refer_out),tuple(agg_out)]+support_out
...
class STSN_fuse_class(nn.Module):
    ...
    def agg1(self,support,reference,score_refer,score_support,test=False):
        ...
        if self.with_modulated_dcn:
            ...
            score_all=torch.zeros_like(score_refer.max(1)[0]).unsqueeze(1)
            for i in range(9):
                ...
                grid=2*torch.cat([pos_x,pos_y],dim=-1)
                # print(grid)
                score_warp=grid_sample(score_support,grid,mode='bilinear').clamp(min=0,max=1)
                feature_warp=grid_sample(support,grid,mode='bilinear').unsqueeze(1)
                score_all=torch.cat([score_all,score_warp.max(1)[0].unsqueeze(1)],dim=1)
                ...
                refer_check=torch.where(score_refer>0.05,self.one,self.zero).max(1)[0]
                warp_check=torch.where(score_warp>0.05,self.one,self.zero).max(1)[0]
                # score_warp=grid_sample(score_support,grid,mode='bilinear').clamp(min=0,max=1)
                # print(torch.max(score_support),torch.max(score_warp))
                # support_class=torch.max(score_warp,dim=1)
                if i==0 and score_support.device==torch.ones(1).cuda(0).device and self.count==0:
                    # print('max support score',torch.max(score_support).item(), \
                    #     'max warp score',torch.max(score_warp).item())
                    print('class check',class_check.sum().item(), \
                        'additional check',((self.one-refer_check)*warp_check).sum().item(), \
                        'betterclass check',(better_check*class_check).sum().item())
                    print('refer result',refer_check.sum().item(), \
                        'warp result',warp_check.sum().item())
                mask_loss=F.binary_cross_entropy(mask[:,i,:,:].sigmoid()*refer_check,class_check.max(1)[0])
                # print('mask loss',mask_loss)
                mask[:,i,:,:]=torch.where((class_check).sum(1)>0,self.one,self.zero)*mask[:,i,:,:]+ \
                    torch.where((self.one-refer_check)*warp_check>0,self.one,self.zero)*mask[:,i,:,:]
                loss+=((torch.where(score_refer>0.05,self.one,self.zero).float()*(score_refer-score_warp).clamp(min=0)).sum() \
                    /(torch.where(score_refer>0.05,self.one,self.zero).float().sum()+1)+mask_loss)/2
                feature=torch.cat([feature,feature_warp],dim=1)
            ...
            mask_softmax=F.softmax(torch.where(mask==self.zero,-1e6*self.one,mask),dim=1)
            mask=torch.where(mask==self.zero,mask,mask_softmax)
            self.mask_weight=torch.max(mask,dim=1)[0].mean().item()
            mask_refer=1-mask.sum(dim=1)
            if score_support.device==torch.ones(1).cuda(0).device and self.count==0:
                print('keep ratio',(mask_refer.sum()/torch.ones_like(mask_refer).sum()).item())
            mask=torch.cat([mask_refer.unsqueeze(1),mask],dim=1).unsqueeze(2)
            out=torch.sum(mask*feature,dim=1)
            score_mask=torch.sum(mask.squeeze(2)*score_all,dim=1)
            # kernel_weight=self.trans_kernel.detach()*9
            # self.conv14.weight=nn.Parameter(kernel_weight)
            # out = self.conv14(support, offset, mask)

            if test:
                return out,offset,mask,score_mask.unsqueeze(1)
            else:
                return out,loss/9,score_mask.unsqueeze(1)
        else:
            # print('agg1',feature_f0.device,self.conv11_offset.weight.device)
            offset1=self.conv11_offset(feature_f0)
    ...
    def fuse(self,datas,score,test=False):
        self.one=self.one.to(datas[0][0].device)
        self.zero=self.zero.to(datas[0][0].device)
        reference=datas[0]
        support=datas[2:]
        score_refer=score[0]
        ...
        for j in range(len(support[0])):
            ...
            refer=reference[j].detach()
            feature=[]
            mask=[]
            for i in range(len(support)):
                refer_check=(score_refer[j].sigmoid()>0.05).float().detach()
                support_check=(score_support[i][j].sigmoid()>0.05).float().detach()
                better_check=(score_support[i][j]>score_refer[j]).float()
                # keep_check=(self.one-better_check)*refer_check
                change_check=(better_check*refer_check).max(1)[0]
                new_check=((self.one-refer_check)*support_check).max(1)[0]
                keep_check=(self.one-change_check)*(self.one-new_check)
                # print(support[i][j].shape,refer.shape,change_check.shape,new_check.shape)
                new_feature=support[i][j].detach()*change_check.unsqueeze(1)+support[i][j].detach()*new_check.unsqueeze(1)+ \
                    refer.detach()*keep_check.unsqueeze(1)
                feature.append(new_feature)
                mask.append((self.one-keep_check)*score_support[i][j].sigmoid().max(1)[0])
            feature=torch.stack(feature,dim=1)
            mask=torch.stack(mask,dim=1)
            warp_check=torch.where(mask>self.zero,self.one,self.zero)
            change_check=torch.where(warp_check.sum(1)>=2,self.one,self.zero).unsqueeze(1).expand_as(feature[:,1,...])
            agg_feature=refer+0
            select_support=(mask*warp_check).max(1)[1].unsqueeze(1).unsqueeze(1).expand_as(feature[:,:1,...])
            index=torch.ones_like(feature[:,:1,...]).long()
            index_b=index*torch.arange(index.shape[0]).view(index.shape[0],1,1,1,1).cuda(index.device)
            index_c=index*torch.arange(index.shape[2]).view(1,1,index.shape[2],1,1).cuda(index.device)
            index_h=index*torch.arange(index.shape[3]).view(1,1,1,index.shape[3],1).cuda(index.device)
            index_w=index*torch.arange(index.shape[4]).view(1,1,1,1,index.shape[4]).cuda(index.device)
            select_feature=feature[index_b,select_support,index_c,index_h,index_w].squeeze(1)
            agg_feature=torch.where(change_check>self.zero,select_feature,agg_feature)
            agg_out.append(agg_feature)
    ...
    def forward(self,datas,class_score,test=False):
        ...
        self.offset_base=self.offset_base.to(datas[0].device)
        output=[]
        self.agg=[self.agg1,self.agg2,self.agg3,self.agg4,self.agg5]
        if datas[0].device=='cuda:0':
            print('fuse channel')
        ...
        for i in range(len(datas)):
            reference=datas[i].detach()+0
            score_refer=class_score[i].sigmoid().detach()+0
            refer_out.append(reference)
            weight0=torch.ones_like(torch.nn.functional.cosine_similarity(reference,reference,dim=1).unsqueeze(1))
            feature=reference.unsqueeze(1)
            mask_check=[]
            for j in range(support_count):
                self.count=j
                shuffle_id=np.random.randint(low=0,high=datas[0].shape[0],size=datas[0].shape[0])
                shuffle_id[shuffle_id==np.arange(datas[0].shape[0])]=shuffle_id[shuffle_id==np.arange(datas[0].shape[0])]-1
                support=datas[i][shuffle_id,:,:,:].detach()+0
                score_support=class_score[i][shuffle_id,:,:,:].sigmoid().detach()+0
                # print(reference.shape,score_support.shape)
                tk_feature,loss,score_mask=self.agg[i](support,reference,score_refer,score_support)
                # print('loss check',loss.item(),i,j)
                weight=torch.nn.functional.cosine_similarity(reference,tk_feature,dim=1).unsqueeze(1)
                weight0=torch.cat([weight0,weight],dim=1)
                feature=torch.cat([feature,tk_feature.unsqueeze(1)],dim=1)
                support_out[j].append(tk_feature)
                losses+=loss
                mask_check.append(score_mask)
            mask_check=torch.cat(mask_check,dim=1)
            warp_check=torch.where(mask_check>self.zero,self.one,self.zero)
            change_check=torch.where(warp_check.sum(1)>2,self.one,self.zero).unsqueeze(1).expand_as(feature[:,1,...])
            agg_feature=reference+0
            # print(weight0[:,1:,...].shape,warp_check.shape)
            select_support=(weight0[:,1:,...]*warp_check).max(1)[1].unsqueeze(1).unsqueeze(1).expand_as(feature[:,:1,...])
            support_feature=feature[:,1:,...]*warp_check.unsqueeze(2)
            index=torch.ones_like(feature[:,:1,...]).long()
            index_b=index*torch.arange(index.shape[0]).view(index.shape[0],1,1,1,1).cuda(index.device)
            index_c=index*torch.arange(index.shape[2]).view(1,1,index.shape[2],1,1).cuda(index.device)
            index_h=index*torch.arange(index.shape[3]).view(1,1,1,index.shape[3],1).cuda(index.device)
            index_w=index*torch.arange(index.shape[4]).view(1,1,1,1,index.shape[4]).cuda(index.device)
            select_feature=support_feature[index_b,select_support,index_c,index_h,index_w].squeeze(1)
            # print(select_feature.shape,change_check.shape,agg_feature.shape)
            agg_feature=torch.where(change_check>self.zero,select_feature,agg_feature)
            # weight=torch.nn.functional.softmax(weight0,dim=1)
            # agg_feature=torch.sum(feature*weight,dim=1)
            # print(agg_feature.shape)
            agg_out.append(agg_feature)
            if i==0 and weight.device==torch.ones(1).cuda(0).device:
                print('max_weight:',torch.max(weight).item(),'mean_max_weight:',torch.max(weight,dim=1)[0].mean().item())
    ...
    def forward_test(self,datas,test=True):
        ...
        for i in range(len(datas)):
            ...
            output.append(agg_feature)
        return tuple(output)
    def forward_eval(self,datas,class_score,test=True):
        output=[]
        self.one=self.one.to(datas[0].device)
        self.zero=self.zero.to(datas[0].device)
        self.offset_base=self.offset_base.to(datas[0].device)
        ...
        for i in range(len(datas)):
            reference=datas[i][:1,:,:,:]+0
            score_refer=class_score[i][:1,:,:,:].sigmoid()+0
            refer_out.append(reference)
            if datas[i].shape[0]>1:
                support=datas[i][1:,:,:,:]+0
                score_support=class_score[i][1:,:,:,:].sigmoid()+0
            else:
                shuffle_id=[0]
                support=datas[i][:1,:,:,:]+0
            score_support=class_score[i][1:,:,:,:].sigmoid()+0
            ...
            for j in range(support.shape[0]):
                self.count=j
                tk_feature,offset,mask,score_mask=self.agg[i](support[j:j+1],reference,score_refer,score_support[j:j+1],test)
                weight=torch.nn.functional.cosine_similarity(reference,tk_feature,dim=1).unsqueeze(1)
                weight0=torch.cat([weight0,weight],dim=1)
                feature=torch.cat([feature,tk_feature.unsqueeze(1)],dim=1)
                support_out[j].append(tk_feature)
                mask_check.append(score_mask)
                self.offset[j].append(offset)
                self.mask[j].append(mask.squeeze(2))
            mask_check=torch.cat(mask_check,dim=1)
            warp_check=torch.where(mask_check>self.zero,self.one,self.zero)
            change_check=torch.where(warp_check.sum(1)>2,self.one,self.zero).unsqueeze(1).expand_as(feature[:,1,...])
            agg_feature=reference+0
...
class STSN_fuse_c(nn.Module):
    #without classfication score check
    def __init__(self,in_channels,out_channels,dcn):
