...
class RepPointsHead(nn.Module):
    ...
    def agg5(self, support, reference, test=False):
        ...
        if self.with_modulated_dcn:
            ...
            feature = self.conv52(support, offset)  # dcn的layer support 相当于是
            return feature, offset
    ...
    def forward_single(self, x, index, test=False):
        ...
        self.agg = [self.agg1, self.agg2, self.agg3, self.agg4, self.agg5]  # 求 offset 和
        ...
        # self.dcn_base_offset 这个变量并不具备反向传播的能力，那么这个变量是怎么得到的呢？
        # rep --- 0,0
        # dcn
        ...
        points_init = 0
        # 这个是reppoints原来的代码
        cls_feat = x  # 这个是分类的feature
        pts_feat = x  # 这个pts_feat 是什么意思？
        # 这一块操作的作用是什么？ 感觉是为了稳定一下特征或者维持一下特征
        ...
        pts_init_feature = self.relu(self.reppoints_pts_init_conv(pts_feat)) # 第一个offset 0 0
        pts_out_init = self.reppoints_pts_init_out(pts_init_feature)  #这里的第一个维度从 256 降低到了 18

        # points_init 是 0 为什么还要加上呢？
        pts_out_init = pts_out_init + points_init
        # refine and classify reppoints
        # control the grad between two loss,0.1 valid grad
        # certain postion
        # 这里为什么要将 pts_out_init detach掉呢？
        # 这一步的操作作用是什么？
        ...
        if not test:
            ...
            weight0 = torch.ones_like(
                torch.nn.functional.cosine_similarity(reference, reference, dim=1).unsqueeze(1).unsqueeze(1))
            feature = reference.unsqueeze(1)
            for j in range(support_count):
                support = cls_out_feature[select_id[j], :, :, :]
                tk_feature, offset = self.agg[index](support, reference)
                # tk_feature = self.agg[index](support, reference)
                weight = torch.nn.functional.cosine_similarity(refer_weight_f, self.cls_weight[index](tk_feature),
            ...
            reference = cls_out_feature[:1, ...] # 获取reference的特征
            refer_weight_f = self.cls_weight[index](reference) # 这行代码的作用是什么？
            # 当前帧和当前帧相似度为1，所以是个全1的矩阵
            weight0 = torch.ones_like(
                torch.nn.functional.cosine_similarity(reference, reference, dim=1).unsqueeze(1).unsqueeze(1))
            feature = reference.unsqueeze(1)
            tmp_offset = []
            for j in range(support_count):
                support = cls_out_feature[j + 1:j + 2, :, :, :] # 这个循环没有问题
                tk_feature, tk_offset = self.agg[index](support, reference)  #这里涉及到3D卷积了
                # refer_weight_f 和 self.cls_weight[index](tk_feature) 是 1 * 2048 * 48 * 156的特征，cos的时候是沿着第一维度进行的
                # unsqueeze(1) 操作两次，那么后面softmax的时候应该是在拼接的维度进行
