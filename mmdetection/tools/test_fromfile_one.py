...
with open(os.path.join(data_path,jsonfile_name),'r',encoding='utf-8') as f:
	data=json.load(f)
out_name='refer'
out_path='/home/ld/RepPoints/debug/reppoint_stsn_warp_three_trainsep/result/epoch_39_thres0.1_nms0.5_with2_refine_offset'
...
# if not os.path.exists(os.path.join(os.path.join(out_path),'agg_result')):
# 		os.mkdir(os.path.join(os.path.join(out_path),'agg_result'))
# if not os.path.exists(os.path.join(os.path.join(out_path),'agg_addition')):
# 		os.mkdir(os.path.join(os.path.join(out_path),'agg_addition'))
...
for i in range(num_classes):
	...
	for j in range(len(cls_gts)):
		...
		if i==2 and len(gbox)>0:
			out_file=os.path.join(os.path.join(out_path),'cycle',classes[i],video_name,img_name.split('/')[-1])
			if not os.path.exists(os.path.join(os.path.join(out_path),'cycle')):
				os.mkdir(os.path.join(os.path.join(out_path),'cycle'))
			if not os.path.exists(os.path.join(os.path.join(out_path),'cycle',classes[i])):
				os.mkdir(os.path.join(os.path.join(out_path),'cycle',classes[i]))
			if not os.path.exists(os.path.join(os.path.join(out_path),'cycle',classes[i],video_name)):
				os.mkdir(os.path.join(os.path.join(out_path),'cycle',classes[i],video_name))
			print(out_file)
			imwrite(img, out_file)
		continue
		img=imshow_det_bboxes(
		...
		img=imshow_det_bboxes(
				img,
				abox,
				np.ones(abox.shape[0]).astype(np.int)*i,
				score_thr=0.1,
				bbox_color='red',
				text_color='red',
				class_names=classes,
