...
for i in range(len(dataset)):
	...
	if 3 in labels:
		...
		check=(labels==3)
		# print(labels)
		# print(data[i]['ann']['labels'])
		imshow_det_bboxes(
				img,
				np.vstack(bboxes)[check,:],
				np.vstack(labels).astype(np.int).squeeze()[check]-1,
				class_names=classes,
				show=False,
				out_file=os.path.join(data_path,'trunk',data[i]['filename'].split('/')[-1]))
		# exit()
