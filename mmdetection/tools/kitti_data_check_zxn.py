...
# 			[8614, 1307,    145,         8897,        509,           681,      417, 532]
count=[]
for i in range(len(classes)):
	count.append(0)
for i in range(len(refer_result)):
	for j in range(len(classes)-3):
		refer_result[i].append(np.array([]))
for i in range(len(refer_result)):
	for j in range(len(classes)-3):
		refer_result[i][j]=np.array([])
out_path='/home/zxn/RepPoints/zxn_result/debug/reppoints_moment_r101_dcn_fpn_kitti_mt/epoch_30_thres0.1_nms0.5_formernms/'
results=[]
video_length=0
...
num_classes = len(classes)  # positive class num
# print(num_classes)
