...
config_file = '/home/zxn/RepPoints/zxn_result/bdd/code/reppoints_moment_r101_dcn_fpn_kitti_agg_fuse_st_baseline_objlevel.py'
checkpoint_file = '/home/ld/RepPoints/ld_result/bdd/epoch_29.pth'
print('test custom zxn kittibddwaymo 65 debug')
...
occlusion_file_name = 'occlusion_test_v.json'  # 'occlusion_test_v.json' or 'occlusion_test_p.json' or 'occlusion_test_vp.json'
level_selected = 'object'  # 'frame' or 'object' or 'objectandframe'
# test a video and show the results

with open('/backdata01/KITTI/kitti/tracking/occlusion_test.json', 'r') as f:
...
if len(datas) == 10:
    ...
    if level_selected == 'objectandframe':
        datas_object = datas.copy()
        datas_object[-1] = datas_object[0] + datas_object[1] + datas_object[2] + datas_object[3] + datas_object[4] + datas_object[5] + datas_object[6] + datas_object[7] + datas_object[8]
...
out_name = 'refer'
out_path = '/home/zxn/RepPoints/zxn_result/debug/reppoints_moment_r101_dcn_fpn_bdd_agg_fuse_st_futhernms/debug_kitti'
...
if test_datasets_version == 'All':
    ...
    for ratio_index, data in tqdm(enumerate(datas)):
        ...
        detection_all = mmcv.load(
            '/home/zxn/RepPoints/zxn_result/debug/reppoints_moment_r101_dcn_fpn_bdd_agg_fuse_st_futhernms/debug_kitti/refer/det_result_objlevel_test_all.pkl')
        groundtruth_all = mmcv.load(
            '/home/zxn/RepPoints/zxn_result/debug/reppoints_moment_r101_dcn_fpn_bdd_agg_fuse_st_futhernms/debug_kitti/refer/gt_selected_objlevel_test_all.pkl')
        loc_all = mmcv.load(
            '/home/zxn/RepPoints/zxn_result/debug/reppoints_moment_r101_dcn_fpn_bdd_agg_fuse_st_futhernms/debug_kitti/refer/loc_result_objlevel_test_all.pkl')
        reppoints_all = mmcv.load(
            '/home/zxn/RepPoints/zxn_result/debug/reppoints_moment_r101_dcn_fpn_bdd_agg_fuse_st_futhernms/debug_kitti/refer/reppoints_test_all.pkl')
        eval_data = []
        loc_data = []
        ...
        for each in data_all_filenames:
            if each[:-11] not in videos_list:
                ...
                print('!!!!!!!!!!!!!')
        import xlsxwriter
        workbook = xlsxwriter.Workbook('Result_1510.xlsx')
        ...
        if level_selected == 'frame' or level_selected == 'objectandframe':
            ...
            for ratio_index, data in tqdm(enumerate(datas_frame)):
                ...
                for i, (frame) in enumerate(data_copy):
                    ...
                    # print(frame['data_index'])
                    result_record_sub.append(detection_all[data_all_filenames.index(frame['filename'])])
                    # temp_frame = groundtruth_all[frame['data_index']]
                    # if level_selected == 'object':
                    #     # 如果是 object level的测试，需要更换非 VV PP VP的label，这里替换为6
                    #     if level_selected == 'object':
                    #         occ_index = frame['occ_index']
                    #         pair_index = frame['occ_index_pair']
                    #         selected_index = occ_index + pair_index
                    #         for i in range(len(frame['ann']['labels'])):
                    #             if i not in selected_index:
                    #                 # print(frame['ann']['labels'][i])
                    #                 frame['ann']['labels'][i] = 6
                    #                 # print(frame['ann']['labels'][i])
                ...
                mmcv.dump(result_record_sub, os.path.join(out_path, out_name, 'det_result_objlevel_%s_%s_%s.pkl' % (
                    occlusion_file_name.split('.')[0].split('_')[-1], level_selected_save, ratio_array[ratio_index])))
