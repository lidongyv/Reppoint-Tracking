...
checkpoint_file = '/home/zxn/RepPoints/zxn_result/kittibddwaymo/stsn_one_do3/epoch_30.pth'
...
out_name = 'baseline'
out_path = '/home/zxn/RepPoints/zxn_result/debug/stsn_one_do3/kittibddwayo_epoch30_thres02_nms05_baseline'
...
if test_datasets_version == 'All':
    ...
    for ratio_index, data in tqdm(enumerate(datas)):
        ...
        for i, (frame) in enumerate(data):
            ...
            embed()
        print('207 debug  before save')
        from IPython import embed
        embed()

        mmcv.dump(img_record, os.path.join(out_path, out_name, 'images_%s_%s.pkl'%(occlusion_file_name.split('.')[0].split('_')[-1], ratio_array[ratio_index])))
        mmcv.dump(result_record, os.path.join(out_path, out_name, 'det_result_%s_%s.pkl'%(occlusion_file_name.split('.')[0].split('_')[-1], ratio_array[ratio_index])))
        mmcv.dump(loc_data, os.path.join(out_path, out_name, 'loc_result_%s_%s.pkl'%(occlusion_file_name.split('.')[0].split('_')[-1], ratio_array[ratio_index])))
        mmcv.dump(offset_data, os.path.join(out_path, out_name, 'offset_%s_%s.pkl'%(occlusion_file_name.split('.')[0].split('_')[-1], ratio_array[ratio_index])))

        print('evaluating result of ', out_name)
        kitti_eval_base(result_record, data)
        ...
        loc_data = []
        import xlsxwriter
        workbook = xlsxwriter.Workbook('Result_stsn_one_02140215_frame_vp.xlsx')
