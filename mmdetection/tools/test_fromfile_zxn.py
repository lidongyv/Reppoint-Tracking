...
config_file = '/home/zxn/RepPoints/zxn_result/retrain_reppoint/code/reppoints_moment_r101_dcn_fpn_kitti_mt_zxn_allclassesnms.py'
print('284 debug')
...
# 这个是为了计算绘制的时间吗？
compute_time = 0
classes = ['Vehicle', 'Pedestrian', 'Cyclist'] # 绘制的时候有三个类别
out_name = ['refer', 'agg']  # refer 是什么意思？

# 这段代码的作用是什么？ 这个应该是之前立冬测试的时候用到的代码吧。
for i in range(10):
    out_name.append('frame_' + str(i + 1))
# out_path='/home/ld/RepPoints/analyze/fuse_result/epoch13 thres0.1'

# 这个是要绘制的Bbox的结果存放的位置。
out_path = '/home/zxn/RepPoints/zxn_result/debug/reppoints_moment_r101_dcn_fpn_kitti_mt/epoch_30_thres0.1_nms0.5_formernms'
results = []
# video_length = 0  # 这个变量没有用
# video_name_check = None  # 这个变量没有用
result_record = [] # 这个变量有用
...
for i in range(len(dataset)):
    ...
    bboxes = ann['bboxes']
    labels = ann['labels']
    gt_bboxes.append(bboxes)  # len 2032
    gt_labels.append(labels)  # len 2032
...
if not os.path.exists(os.path.join(os.path.join(out_path), 'drawing')):
    os.mkdir(os.path.join(os.path.join(out_path), 'drawing'))
...
for j in range(len(gt_bboxes)):
    ...
    if not os.path.exists(os.path.join(os.path.join(out_path), 'drawing', video_name)):
        os.mkdir(os.path.join(os.path.join(out_path), 'drawing', video_name))
    for k in range(len(rbox)):
        imshow_det_bboxes(
            img,
            rbox[k],
            np.ones(gbox.shape[0]).astype(np.int) * k,
            class_names=classes[k],
            show=False,
            out_file=os.path.join(os.path.join(out_path), 'drawing', video_name, img_name.split('/')[-1]))
    print('367 debug')
    from IPython import embed
    embed()
...
# img = os.path.join(data_path, data[0]['filename']) #原始图像的位置
