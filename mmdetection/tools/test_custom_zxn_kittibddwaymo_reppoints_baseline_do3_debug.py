...
jsonfile_name = 'kitti_bdd_waymo_2class_val.json'
occlusion_file_name = ['kitti_bdd_waymo_2class_val_v.json', 'kitti_bdd_waymo_2class_val_p.json', 'kitti_bdd_waymo_2class_val_vp.json']
level_selected = ['frame', 'object']

test_datasets_version = 'Sub'  #'All' or 'Sub'
...
if test_datasets_version == 'All':
    ...
    for ratio_index, data in tqdm(enumerate(datas)):
        ...
        reppoint_data = [[] for i in range(5)]
        for each_occlusion_file_index, each_occlusion_file in enumerate(occlusion_file_name):

            with open(data_path + each_occlusion_file, 'r') as f:
                datas = json.load(f)
            datas_object = datas.copy()
            datas_object[-1] = datas_object[0] + datas_object[1] + datas_object[2] + datas_object[3] + \
                               datas_object[4] + datas_object[5] + datas_object[6] + datas_object[7] + \
                               datas_object[8]
            ...
            for each_level_index, each_level in enumerate(level_selected):
                for ratio_index, data in tqdm(enumerate(datas_object)):
                    ...
                    # mmcv.dump(reppoint_data, os.path.join(out_path, out_name, 'reppoints_%s_%s.pkl' % (
                    #     occlusion_file_name.split('.')[0].split('_')[-1], ratio_array[ratio_index])))
                    mean_ap, eval_results = kitti_eval(result_record_sub, data_selected_sub, each_level)
