...
def bdd_eval(det_results, dataset, iou_thr=0.5):
    gt_bboxes = []
    gt_labels = []
    gt_ignore = []
...
data_path='/data/aidata/waymo/formal'
data_root_path='/data/aidata/waymo/formal'

# jsonfile_name='occlusion_val_waymo_zxn_02110210_v.json'
#
...
# gt_bboxes = []
# gt_labels = []
# output_path='/home/zxn/RepPoints/occ_data_waymo/waymo/test_v'
...
#     for j in select:
#         img_name = data[i][j]['filename'][49:]
#         occ_index = data[i][j]['occ_index']
#         boxes = data[i][j]['ann']['bboxes']
#         labels = data[i][j]['ann']['labels']
#         pair_index = data[i][j]['occ_index_pair']
#         img = os.path.join(data_path, 'val', img_name)
#
#         index = occ_index + pair_index
#
#         imshow_det_bboxes(
#             img,
#             np.vstack(boxes)[index],
#             np.vstack(labels).astype(np.int).squeeze()[index] - 1,
#             class_names=classes,
#             show=False,
#             out_file=os.path.join(save_path, img_name.split('/')[-1]))
#
# # jsonfile_name = 'occlusion_test_p.json'
# jsonfile_name='occlusion_val_waymo_zxn_02110210_p.json'
# # test a video and show the results
# with open(os.path.join(data_path, jsonfile_name), 'r', encoding='utf-8') as f:
#     data = json.load(f)
#
...
# gt_bboxes = []
# gt_labels = []
# output_path='/home/zxn/RepPoints/occ_data_waymo/waymo/test_p'
# # output_path = '/home/zxn/RepPoints/occ_data/bdd/test_p'
# if not os.path.exists(output_path):
#     os.mkdir(output_path)
# ratio = (np.arange(1, 11).astype(np.float) / 10).tolist()
# for i in range(10):
...
jsonfile_name='occlusion_val_waymo_zxn_02110331_vp.json'
# test a video and show the results
with open(os.path.join(data_path, jsonfile_name), 'r', encoding='utf-8') as f:
    data = json.load(f)
...
gt_bboxes = []
gt_labels = []
# output_path = '/home/zxn/RepPoints/occ_data/bdd/test_vp'
output_path='/home/zxn/RepPoints/occ_data_waymo/waymo/test_vp'
if not os.path.exists(output_path):
    os.mkdir(output_path)
ratio = (np.arange(1, 11).astype(np.float) / 10).tolist()
for i in range(10):
