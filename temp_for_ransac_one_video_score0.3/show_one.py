...
if __name__=='__main__':
    ...
    data=mmcv.load(os.path.join(root_path, 'json', 'waymo_one_video.json'))
    loc_record=mmcv.load(os.path.join(root_path, 'pkl', 'loc_result_video_all.pkl'))
    det_record = mmcv.load(os.path.join(root_path, 'pkl', 'det_result_video_all.pkl'))
    all_imgname_list = []
    for each in data:
    ...
    for each_flow_name in flow_dir_names:
        ...
        show_img2=img2+0
        print(img1.shape)
        loc1=torch.from_numpy(np.vstack(loc_record[current_index]))
        loc2=torch.from_numpy(np.vstack(loc_record[current_index + 1]))
        offset1_ori=torch.from_numpy(mmcv.load(os.path.join(root_path, 'flow', '%s/'%each_flow_name, 'offset1_ori.pkl')))
        offset2_ori=torch.from_numpy(mmcv.load(os.path.join(root_path, 'flow', '%s/'%each_flow_name, 'offset2_ori.pkl')))
        offset1_sample=torch.from_numpy(mmcv.load(os.path.join(root_path, 'flow', '%s/'%each_flow_name, 'offset1_sample.pkl')))
        offset2_sample=torch.from_numpy(mmcv.load(os.path.join(root_path, 'flow', '%s/'%each_flow_name, 'offset2_sample.pkl')))
        ...
        ps=8
        classes=['car','person']
        if not os.path.exists(os.path.join(root_path, 'flow', '%s/'%each_flow_name,'visualize')):
            os.mkdir(os.path.join(root_path, 'flow', '%s/'%each_flow_name,'visualize'))
        out_path=os.path.join(root_path, 'flow', '%s/'%each_flow_name,'visualize')
