...
class STSN_one(nn.Module):
    ...
    def agg1(self,support,reference,test=False):
        ...
        if self.with_modulated_dcn:
            ...
            offset=self.conv11_offset(feature_f0)

            feature_f1=self.conv11(feature_f0,offset)

            offset=self.conv12_offset(feature_f1)

            feature_f2=self.conv12(feature_f1,offset)

            offset=self.conv13_offset(feature_f2)

            feature_f3=self.conv13(feature_f2,offset)
    ...
    def forward(self,datas,test=False):
        ...
        k=1
        out=[]
        select_id[0]=torch.arange(datas[0].shape[0])-k
        select_id[0]=torch.where(select_id[0]<0,torch.arange(datas[0].shape[0]),select_id[0])
        select_id[1]=torch.arange(datas[0].shape[0])+k
        ...
        for i in range(len(datas)):
            ...
            for j in range(support_count):
                ...
                tk_feature,offset=self.agg[i](support,reference)
