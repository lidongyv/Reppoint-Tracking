...
class RepPointsHead(nn.Module):
    ...
    def forward_single(self, x,index):
        ...
        # refer_weight_f=self.weight_feature[index](reference)
        ...
        #     weight=torch.nn.functional.cosine_similarity(refer_weight_f,self.weight_feature[index](tk_feature),dim=1).unsqueeze(1).unsqueeze(1)
        #     weight0=torch.cat([weight0,weight],dim=1)
        #     feature=torch.cat([feature,tk_feature.unsqueeze(1)],dim=1)
        # weight=torch.nn.functional.softmax(weight0[:,1:3,...],dim=1)
        # agg_feature=torch.sum(feature[:,1:3,...]*weight,dim=1)
        ...
        # agg_pts_out_refine = agg_pts_out_refine + pts_out_init.detach()
