...
def plot_according_to_point(vis_attr, im, source_points, map_h, map_w, loc_offset_scale, color=[0, 0, 255]):
    ...
    # 红色，绿色，蓝色；橙色，紫红色，湖蓝色；深紫色，深湖蓝色，深绿色；
    # 紫色，草绿色，紫红色，棕色，海淀蓝，红色，蓝色，湖蓝色
    # 颜色的顺序是 BGR [255, 0, 0]是蓝色；[0, 0, 255]是蓝色
    color_list = [[0, 0, 255], [0, 255, 0], [255, 0, 0],
                  [0, 125, 255], [255, 0, 255], [255, 255, 125],
                  [125, 0, 125], [125, 125, 0], [57, 117, 57]]
    # 橙色， 紫色，草绿色，紫红色，棕色，海淀蓝，红色，蓝色，湖蓝色
    # color_list = [[49, 125, 137], [219, 84, 125], [100, 202, 175],
    #               [232, 70, 219], [71, 121, 197], [211, 151, 57],
    #               [92, 37, 231], [219, 57, 49], [242, 239, 26]]

    # color_list = [[137, 125, 49], [125, 84, 219], [175, 202, 100],
    #               [219, 70, 232], [197, 121, 71], [57, 151, 211],
    #               [231, 37, 92], [49, 57, 219], [26, 239, 242]]
    for idx, cur_source_point in enumerate(source_points):
        ...
        im[y - plot_area:y + plot_area + 1, x - plot_area:x + plot_area + 1, :] = np.tile(
            np.reshape(color_list[idx], (1, 1, 3)), (2 * plot_area + 1, 2 * plot_area + 1, 1)
        )
    return im
...
def show_dconv_offset_by_loc(im, all_offset, path, loc, loc_offset_scale, step=[2, 2], filter_size=3,
    ...
    count = 0
    for loc_index, (im_w, im_h) in enumerate(loc):
        ...
        # path_each = path + '%03d.jpg'%loc_index
...
if __name__ == '__main__':
    ...
    #加载某一个 Reppoints
    reppoints = mmcv.load(
        '/home/zxn/RepPoints/zxn_result/debug/repoint_baseline_do3/waymo_epoch20_thres01_nms05_baseline_0218_sub8/baseline/reppoints_val_all.pkl')
    #这个path是用来干什么的？
    path = '/home/zxn/RepPoints/zxn_result/debug/repoint_baseline_do3/waymo_epoch20_thres01_nms05_baseline_0218_sub8/baseline'
    classes = ['Car', 'Person', 'Cyclist']
    # split = ['right_detected', 'not_detected', 'wrong_detected'] # 这个是在什么时候弄好的？
        ...
        for j in range(len(video_name)):
            ...
            for m in range(len(split)):
                ...
                for n in range(len(pkls)):
                    ...
                    name = name.split('/')
                    offset_path = video_split + '/offset_each'
                    if not os.path.exists(offset_path):
                        os.mkdir(offset_path)
                    offset_path = os.path.join(offset_path, name[-1])

                    if split[m] == 'wrong_detected' or split[m] == 'right_detected':
