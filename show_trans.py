	...
	for i in range(6):
		...
		for m in range(5):
			...
			if not os.path.exists(save_path):
				...
				os.mkdir(save_path)
			offset=[np.load(path+'stage_'+str(m)+'_offset_0.npy')]
			weight=[np.load(path+'stage_'+str(m)+'_offset_1.npy')]
			trans=[np.load(path+'stage_'+str(m)+'_offset_2.npy')]
			loc_select=loc[np.where(loc[:,2]==scale[m]),:2]
			# loc_select=loc[np.where(loc[:,2]>0),:2]
			show_mask_dcn_by_loc(imgs,offset,save_path,loc_select[0],plot_level=len(offset),plot_area=3)
