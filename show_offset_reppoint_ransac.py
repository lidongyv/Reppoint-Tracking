...
if __name__ == '__main__':
	...
	#加载某一个 Reppoints
	reppoints = mmcv.load(
		'/home/ld/RepPoints/ld_result/stsn_one_ransac_do3/reppoint/epoch_23_thres0.3_nms0.5_support_5remask/agg/reppoints.pkl')
	masks = mmcv.load(
		'/home/ld/RepPoints/ld_result/stsn_one_ransac_do3/reppoint/epoch_23_thres0.3_nms0.5_support_5remask/agg/mask_new.pkl')
	#这个path是用来干什么的？
	path = '/home/ld/RepPoints/ld_result/stsn_one_ransac_do3/reppoint/epoch_23_thres0.3_nms0.5_support_5re/agg'
	classes = ['Car', 'Person']
	split = [ 'addition','not_detected'] # 这个是在什么时候弄好的？
		...
		for j in range(len(video_name)):
			...
			for m in range(len(split)):
				...
				for n in range(len(pkls)):
					...
					reppoint_frame = reppoints[0][index]  #提取出来的shape是：1 * 18 * 48 * 156  这个像是某一层的特征
					mask_frame=masks[0][index]
					...
					offset_path = video_split + '/ransac'
					if not os.path.exists(offset_path):
						os.mkdir(offset_path)
					offset_path = os.path.join(offset_path, name[-1])
					show_dconv_offset_by_loc(img, [mask_frame],  offset_path, loc, loc_offset_scale, plot_level=1, plot_area=3)
			# exit()
