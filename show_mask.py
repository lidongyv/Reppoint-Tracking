...
def plot_according_to_point(vis_attr, im, source_points, map_h, map_w, color=[255,0,0],mask=None):
...
def show_dconv_offset_by_loc(im, all_offset, path,loc,step=[2, 2], filter_size=3,
	...
	for (im_w,im_h) in loc:
		...
		cur_im = plot_according_to_point(vis_attr, cur_im, source_points, map_h, map_w,mask=mask[:,im_h.astype(np.int),im_w.astype(np.int)])
		cur_im[source_y-4:source_y+4+1, source_x-4:source_x+4+1, :] = \
			np.tile(np.reshape([255, 255, 0], (1, 1, 3)), (2*4+1, 2*4+1, 1))
		# cur_im[target_y-4:target_y+4+1, target_x-4:target_x+4+1, :] = \
						# np.tile(np.reshape([0, 0, 255], (1, 1, 3)), (2*4+1, 2*4+1, 1))
...
if __name__=='__main__':
	...
	for i in range(6):
		...
		for m in range(5):
			...
			mask=np.load(path+'/mask_'+str(m)+'.npy').squeeze()
			...
			show_dconv_offset_by_loc(imgs,[offset4],save_path,loc_select[0],plot_level=1,plot_area=3,mask=mask)
		# exit()
