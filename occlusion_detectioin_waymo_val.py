...
def ratio_check(occluded):
	...
	return count,index
jsonfile_name='waymo_val_2class.json'
...
for i in range(len(v_video)):
	...
	if len(v_video[i]['occluded']) / v_video[i]['length'] > 0.0 or vvcase_count[0] * 0 < np.sum(vvcase_count[1:]):
...
for i in range(len(p_video)):
	...
	if len(p_video[i]['occluded']) / p_video[i]['length'] > 0.0 or ppcase_count[0] * 0 < np.sum(ppcase_count[1:]):
...
for i in range(len(vp_video)):
	...
	if len(vp_video[i]['occluded']) / vp_video[i]['length'] > 0.0 or vpcase_count[0] * 0 < np.sum(vpcase_count[1:]):
...
f1 = open("/data/aidata/waymo/formal/analysis/script/selected_val.txt",'r')
selected_val_names_54 = f1.read().split('\n')[:-1]
print(selected_val_names_54)

test_fuse=[]
for i in range(len(test_all)):
	if test_all[i][0] in selected_val_names_54:
	# if (test_all[i][0] not in test_fuse) and (test_all[i][0] in selected_val_names_54):
		test_fuse.append(test_all[i][0])

# 看看总共有多少个视频段，应该有重合的
temp_video_alls = []
for each_video in test_fuse:
	if each_video not in temp_video_alls:
		temp_video_alls.append(each_video)
	else:
		print('!!!!!!!!!!!!!')
print('366')
...
mmcv.dump(test_data_all,os.path.join(data_path,'occlusion_val_waymo_zxn_02110331.json'))
# mmcv.dump(train_data_all,os.path.join(data_path,'occlusion_train.json'))
mmcv.dump(test_data_v,os.path.join(data_path,'occlusion_val_waymo_zxn_02110331_v.json'))
mmcv.dump(test_data_p,os.path.join(data_path,'occlusion_val_waymo_zxn_02110331_p.json'))
mmcv.dump(test_data_vp,os.path.join(data_path,'occlusion_val_waymo_zxn_02110331_vp.json'))

print('frame count')
# print('train_data_all:',len(train_data_all),'test_data_all:',len(test_data_all))
print('test_data_all:',len(test_data_all))
